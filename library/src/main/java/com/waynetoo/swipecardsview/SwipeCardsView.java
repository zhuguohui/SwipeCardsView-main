...
public class SwipeCardsView extends LinearLayout {
    ...
    private final int touchSlopSqure;
    ...
    public SwipeCardsView(Context context, AttributeSet attrs, int defStyle) {
        ...
        touchSlopSqure = mTouchSlop * mTouchSlop;
...
//            LogUtil.d("test notifyDatasetChanged can not Reset View=" + index);
    ...
    MoveDirection lastMoveDirection = MoveDirection.DOWN;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!isIntercepted) {
                    int distanceX = currentX - mInitialMotionX;
                    int distanceY = currentY - mInitialMotionY;

                    boolean moveY = Math.abs(distanceY) > mTouchSlop;
                    boolean moveX = Math.abs(distanceX) > mTouchSlop;
                    ...
                    if (moveX || moveY) {
                        ...
                        if (resetDirection) {
                            ...
                            lastMoveDirection = getMoveDirection(distanceX, distanceY);
                            //对方向进行约束
                        }
                    } else {
                        return super.dispatchTouchEvent(ev);
...
//                    sendCancelEvent();
    ...
    /**
     * 返回true 表示交由 supper.dispatchTouchEvent处理。
     *
     * @param currentY
     * @return
     */
    private boolean dealMoveYEvent(int currentY) {
    ...
    enum MoveDirection {
        ...
        MoveDirection(boolean horizontal) {
    ...
    private void moveTopView(int deltaX, int deltaY) {
        View topView = getTopView();
        if (topView != null) {
            if (lastMoveDirection == MoveDirection.DOWN && topView.getTop() >= initTop) {
                return;
            }
            ...
            //透明度处理
            int distance = lastMoveDirection.isHorizontal() ? (initLeft + mCardWidth) : (initTop + mCardHeight);
            float moveDis = lastMoveDirection.isHorizontal() ? (initLeft - topView.getLeft()) : (initTop - topView.getTop());
...
//        child.setAlpha(alpha);
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            int dx = x - topView.getLeft();
            int dy = y - topView.getTop();
            if (lastMoveDirection.isHorizontal()) {
                dy = 0;
            } else {
                dx = 0;
            }
            if (y != mScroller.getFinalY() || x != mScroller.getFinalX()) {
                moveTopView(dx, dy);
            }
            ViewCompat.postInvalidateOnAnimation(this);
        } else {
...
//            viewList.get(viewList.size() - 1).setVisibility(View.GONE);
    ...
    private void processLinkageView(View changedView, float alpha) {
        ...
        boolean isPull = lastMoveDirection == MoveDirection.DOWN;
        if (isPull) {
        ...
        for (int i = 1; i < viewList.size(); i++) {
            ...
            adjustLinkageViewItem(changedView, rate3, alpha, i);
        }
    }

    // 由index对应view变成index-1对应的view
    private void adjustLinkageViewItem(View changedView, float rate, float v, int index) {
//        Log.i(TAG, "ajustLinkageViewItem: rate=" + rate + "  alpha=" + v);
    ...
    private void onTopViewReleased(View changedView, float xvel, float yvel) {
        ...
        if (!lastMoveDirection.isHorizontal()) {
            if (lastMoveDirection == MoveDirection.UP) {//向上滑出
                finalY = -mCardHeight;
                flyType = SlideType.TOP;
                addReleaseList = true;
            } else if (lastMoveDirection == MoveDirection.DOWN) {//向下拉出来
                finalY = initTop;
                flyType = SlideType.BOTTOM;
            }
        } else {
            if (lastMoveDirection == MoveDirection.LEFT) {
                finalX = -mCardWidth;
